...
Run: streamlit run aquasentinel_monolithic.py
"""
    ...
    if nodes:
        ...
        for u, v, data in cascade.graph.edges(data=True):
            ...
            folium.PolyLine(
                ...
                weight=4,
                opacity=0.8,
                dash_array='5, 10' if u_risk > 0 else None,
                tooltip=f"Channel {u} ➔ {v} (Routing Lag: {lag} min)"
            ).add_to(m)
        ...
        for n in nodes:
            ...
            <div style="font-family: Arial; min-width: 180px;">
                <h4 style="margin: 0 0 5px 0;">{n['name']} ({n['id']})</h4>
                <b>Type:</b> {n['type'].upper()}<br>
                <b>Elevation:</b> {n['elevation_m']} m<br>
                <b>Lat/Lon:</b> {n['lat']}, {n['lon']}<br>
                <b>Threat State:</b> <span style="color:{color_name}; font-weight:bold;">{n['risk_state']}</span><br>
                <b>Accumulated Q:</b> {n['total_discharge']} m³/s
            </div>
            """
            ...
            folium.Marker(
                ...
                tooltip=f"{n['id']}: {n['risk_state']} ({n['total_discharge']} m³/s)",
                icon=folium.Icon(color=color_name, icon=icon_name, prefix='fa')
            ).add_to(m)
            ...
            if r_level == 2:
                folium.Circle(
                    ...
                    fill=True,
                    fill_opacity=0.25,
                    popup=f"High Risk Inundation Hazard: {n['id']}"
                ).add_to(m)
